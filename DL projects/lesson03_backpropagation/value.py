...
class Value:
    ...
    def __add__(self, other):
        # Create a new node that represents the sum of two Value objects.
        out = Value(
        ...
        # When gradients flow back through addition, each parent gets the same
        # upstream gradient because d(x + y)/dx = 1 and d(x + y)/dy = 1.
        def _backward():
            self.grad += out.grad
            other.grad += out.grad

        # Attach the local backward rule to the output node.
